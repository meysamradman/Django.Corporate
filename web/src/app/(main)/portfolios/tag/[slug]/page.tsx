...
async function PortfoliosTagBody({ params, searchParams }: PageProps) {
  ...
  if (normalizedSlug !== slug) {
    permanentRedirect(buildPortfolioListHref({ page, search, tag_slug: normalizedSlug, category_slug: undefined }));
  }
...
export default function PortfolioTagPage({ params, searchParams }: PageProps) {
  return (
    <main className="container mx-auto px-4 py-10 md:py-12">
      <div className="mb-8">
        <h1 className="mb-3 text-3xl font-bold text-font-p md:text-4xl">نمونه‌کارها</h1>
        <p className="max-w-2xl text-font-s">نمایش نمونه‌کارها براساس تگ انتخاب‌شده.</p>
      </div>

      <Suspense fallback={<PortfoliosPageFallback />}>
        <PortfoliosTagBody params={params} searchParams={searchParams} />
      </Suspense>
    </main>
  );
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: `نمونه‌کارها | تگ ${slug}`,
    description: `نمونه‌کارها با تگ ${slug}.`,
  };
}