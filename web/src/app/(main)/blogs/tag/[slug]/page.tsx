...
async function BlogsTagBody({ params, searchParams }: PageProps) {
  ...
  if (normalizedSlug !== slug) {
    permanentRedirect(buildBlogListHref({ page, search, tag_slug: normalizedSlug, category_slug: undefined }));
  }

  const response = await blogApi
    .getBlogList(toBlogListApiParams({ page, search, tag_slug: normalizedSlug }))
    .catch(() => null);
  ...
  return (
    <main className="container mx-auto px-4 py-10 md:py-12">
      <div className="mb-8">
        ...
        <p className="max-w-2xl text-font-s">نمایش مطالب براساس تگ انتخاب‌شده.</p>
      </div>

      <Suspense fallback={<BlogsPageFallback />}>
        <BlogsTagBody params={params} searchParams={searchParams} />
      </Suspense>
    </main>
  );
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: `وبلاگ | تگ ${slug}`,
    description: `مطالب وبلاگ با تگ ${slug}.`,
  };
}
