...
export default function PropertyRequestFormClient() {
  ...
  const onSubmit = async (values: DynamicFormValues) => {
    ...
    if (values.__hp && String(values.__hp).trim()) {
      ...
      setSubmitMessage("درخواست شما با موفقیت ثبت شد.");
      form.reset({ __hp: "" });
      return;
    }
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        setSubmitMessage(err.response.message || "خطا در ثبت درخواست");
        return;
      }

      setSubmitState("error");
      setSubmitMessage("خطا در ثبت درخواست");
    }
  };
  ...
  return (
    <section className="bg-wt border border-br/50 rounded-2xl overflow-hidden">
      <div className="px-6 py-5 border-b border-br/50">
        <h2 className="text-base font-black">فرم درخواست ملک</h2>
        <p className="mt-1 text-sm text-font-s">لطفاً اطلاعات موردنیاز را ثبت کنید تا کارشناسان ما با شما تماس بگیرند.</p>
      </div>
      ...
      <div className="p-6">
        {loadingFields ? (
          ...
          <div className="rounded-xl border border-br/50 bg-bg p-4 text-sm text-font-s leading-7">
            برای نمایش فرم، ابتدا فیلدهای فرم را در بک‌اند تعریف کنید.
            <br />
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  render={({ field: rhfField, fieldState }) => {
                    return (
                      <div className={field.field_type === "textarea" ? "md:col-span-2" : ""}>
                        ...
                        <div className="mt-1.5">
                          <div>
                            {React.cloneElement(renderField(field) as any, {
                              ...rhfField,
                              value: field.field_type === "checkbox" ? undefined : rhfField.value ?? "",
                              checked: field.field_type === "checkbox" ? !!rhfField.value : undefined,
                              onChange: (e: any) => {
                                if (field.field_type === "checkbox") {
                        ...
                        {fieldState.error?.message ? <p className="mt-1 text-sm text-red-2">{fieldState.error.message}</p> : null}
                      </div>
                    );
                  }}
                />
            ...
            <div className="flex items-center justify-end gap-3">
              <Button type="submit" disabled={submitState === "submitting"}>
                {submitState === "submitting" ? "در حال ثبت..." : "ثبت درخواست"}
              </Button>
            </div>
          </form>
        )}
