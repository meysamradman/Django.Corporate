...
export function Breadcrumb() {
    ...
    return (
        <nav aria-label="Breadcrumb" className="flex items-center space-x-1 text-sm text-font-a h-full py-2">
            {displayItems.map((item, index) => (
                <React.Fragment key={index}>
                    {index > 0 && (
                        <ChevronRight 
                            className="h-4 w-4 text-muted-foreground/50 rotate-180" 
                        />
                    )}
                    ...
                    {item.label === "..." ? (
                        ...
                        <Link
                            href={item.href}
                            className="hover:text-foreground transition-colors duration-200 font-normal"
                        >
                            {item.label}
                        </Link>
                    )}
