...
export function PortfolioStatus({
    ...
    const getStatusValue = (field: string, defaultValue: any) => {
        if (isFormApproach) return watch?.(field as any) ?? defaultValue;
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
            <Item className="p-2.5 h-16 rounded-xl border-blue-1/20 bg-blue-0/10 flex-row items-center gap-3">
                ...
                <span className="flex-1 text-[11px] font-bold text-blue-2 leading-tight">نمایش عمومی</span>
                <ItemActions>
                    <Switch
                        checked={getStatusValue("is_public", true)}
                        disabled={!editMode}
            ...
            <Item className="p-2.5 h-16 rounded-xl border-green-1/20 bg-green-0/10 flex-row items-center gap-3">
                ...
                <span className="flex-1 text-[11px] font-bold text-green-2 leading-tight">وضعیت فعال</span>
                <ItemActions>
                    <Switch
                        checked={getStatusValue("is_active", true)}
                        disabled={!editMode}
            ...
            <Item className="p-2.5 h-16 rounded-xl border-orange-1/20 bg-orange-0/10 flex-row items-center gap-3">
                ...
                <span className="flex-1 text-[11px] font-bold text-orange-2 leading-tight">وضعیت ویژه</span>
                <ItemActions>
                    <Switch
                        checked={getStatusValue("is_featured", false)}
                        disabled={!editMode}
            ...
            <Item className="p-2.5 h-16 rounded-xl border-purple-1/20 bg-purple-0/10 flex-row items-center gap-3">
                ...
                <span className="flex-1 text-[11px] font-bold text-purple-2 leading-tight">منتشر شده</span>
                <ItemActions>
                    <Switch
                        checked={isFormApproach ? (watch?.("status" as any) === "published") : (formData?.status === "published")}
                        disabled={!editMode}
