        ...
        >
            {!hasSEO ? (
                <EmptyState
                    ...
                    description="توضیحات متا و پیش‌نمایش شبکه‌های اجتماعی برای این پروژه ثبت نشده است"
                    icon={Globe}
                    size="sm"
                    fullBleed={true}
                />
                        ...
                        <Card className="flex-1 bg-bg border-br/40 shadow-xs group transition-smooth hover:border-teal-1/30 overflow-hidden relative">
                            ...
                            <CardContent className="p-6 space-y-3 pt-8">
                                <div className="flex items-center gap-3 text-sm text-font-s opacity-80 truncate">
                                    ...
                                    <span className="dir-ltr text-left flex-1 truncate font-medium">{canonicalUrl || `portfolio/${portfolio.slug}`}</span>
                                </div>
                                ...
                                <p className="text-base text-[#4d5156] line-clamp-2 leading-relaxed">
                                    <span className="font-bold text-font-s opacity-60 ml-2">
                                        {portfolio.created_at ? new Date(portfolio.created_at).toLocaleDateString('fa-IR') : new Date().toLocaleDateString('fa-IR')} —
                                    </span>
                                    {metaDescription}
                                </p>
                            </CardContent>
