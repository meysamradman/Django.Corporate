...
import { Image as ImageIcon, X, ChevronLeft, ChevronRight, Play } from "lucide-react";
...
export function PortfolioImageGallery({ portfolio, className }: PortfolioImageGalleryProps) {
    ...
    return (
      <div className={cn("relative w-full h-[500px] rounded-xl border border-br bg-bg flex items-center justify-center", className)}>
        <div className="text-center space-y-4">
          ...
          <p className="text-font-s">تصویر یا ویدئویی برای این نمونه‌کار آپلود نشده است</p>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <Card className={className}>
        ...
        <div className="relative w-full h-[500px] overflow-hidden bg-bg group">
          ...
          {/* Counter */}
          <div className="absolute bottom-4 left-4 px-3 py-1.5 rounded-lg bg-static-b/80 backdrop-blur-sm text-static-w text-sm font-medium z-10">
          ...
          {mediaItems.length > 1 && (
            <>
              <button
                onClick={scrollNext}
                className="absolute left-4 top-1/2 -translate-y-1/2 size-10 rounded-full bg-static-b/80 backdrop-blur-sm text-static-w hover:bg-static-b border-none shadow-lg flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
              ...
              <button
                onClick={scrollPrev}
                className="absolute right-4 top-1/2 -translate-y-1/2 size-10 rounded-full bg-static-b/80 backdrop-blur-sm text-static-w hover:bg-static-b border-none shadow-lg flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
      ...
      {isLightboxOpen && (
        <div className="fixed inset-0 z-50 bg-static-b/95 backdrop-blur-md flex items-center justify-center">
          <button
            ...
            className="absolute top-4 left-4 z-50 p-2 rounded-lg bg-static-w/20 hover:bg-static-w/30 text-static-w transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
          ...
          {mediaItems.length > 1 && (
            <>
              <button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 size-12 rounded-full bg-static-w/20 hover:bg-static-w/30 text-static-w flex items-center justify-center transition-colors"
              >
                <ChevronRight className="w-8 h-8" />
              </button>
              <button
                onClick={() => lightboxApi?.scrollNext()}
                className="absolute left-4 top-1/2 -translate-y-1/2 size-12 rounded-full bg-static-w/20 hover:bg-static-w/30 text-static-w flex items-center justify-center transition-colors"
              >
