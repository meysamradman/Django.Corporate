/**
 * Portfolio Print View Component
 * 
 * Precision Mirror View: Matches the Portfolio Management Table columns exactly.
 * Columns: Title, Status, Featured, Public, Categories, Created At, Active.
 */

import { useQuery } from '@tanstack/react-query';
import { format } from 'date-fns-jalali';
import { getPortfoliosByIds } from '@/api/portfolios/portfolios';
...
export function PortfolioPrintView({ portfolioIds }: PortfolioPrintViewProps) {
    const { data: portfolios, isLoading, error } = useQuery({
        queryKey: ['portfolios', 'print', portfolioIds],
        queryFn: () => getPortfoliosByIds(portfolioIds),
    ...
    const COLUMNS = [
        ...
        },
        {
            ...
            width: '24%',
            render: (item: any) => (
                <div className="text-right font-bold text-sm text-slate-900">{item.title}</div>
            )
        },
        {
            key: 'status',
            ...
            align: 'center' as const,
            render: (item: any) => (
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${item.status === 'published' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                    }`}>
        ...
        },
        {
            ...
            align: 'center' as const,
            render: (item: any) => (
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${item.is_featured ? 'bg-orange-100 text-orange-800' : 'bg-gray-100 text-gray-800'
                    }`}>
        ...
        },
        {
            ...
            align: 'center' as const,
            render: (item: any) => (
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${item.is_public ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                    }`}>
        ...
        },
        {
            ...
            render: (item: any) => (
                <span className="text-xs font-bold text-slate-700">
                    {toPersianDigits(format(new Date(item.created_at), 'yyyy/MM/dd'))}
                </span>
            )
        },
        {
            ...
            align: 'center' as const,
            render: (item: any) => (
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${item.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                ...
                <span className="mr-3 text-gray-600 font-bold">درحال بارگذاری لیست پروژه‌ها...</span>
            </div>
        );
    }
            ...
            <div className="print-container pt-20 pb-12 px-10 mx-auto xl:max-w-[297mm]">
                <div className="pb-8 mb-8 border-b-2 border-slate-900 flex justify-between items-end">
                    <div className="text-right">
                        <h1 className="text-3xl font-black text-slate-900 mb-2">گزارش جامع لیست پروژه‌ها</h1>
                        <p className="text-sm text-slate-400 font-bold italic tracking-widest uppercase">System Export / Engineering Report</p>
                    </div>
                    <div className="text-left text-xs text-slate-700 font-bold space-y-1">
                        ...
                        <div>تعداد کل: {toPersianDigits(portfolios?.length || 0)} مورد</div>
                    </div>
                </div>
