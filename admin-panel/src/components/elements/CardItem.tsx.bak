...
import { Card, CardContent, CardHeader, CardAction } from "@/components/elements/Card";
...
function CardItemComponent<T = any>({
  ...
  return (
    ...
    >
      <CardHeader className="flex flex-row items-center justify-between pt-5 px-6 pb-0">
        <div data-slot="card-status">
          {statusLabel && (
            <span
              className={cn(
        ...
        {availableActions.length > 0 && (
          <CardAction>
            <DropdownMenu>
              ...
              >
                <button
                  className="flex items-center justify-center size-8 rounded-md bg-gray-0/30 hover:bg-gray-0/50 transition-colors focus:outline-none"
                >
      ...
      <CardContent className="px-6 pt-4 pb-6 flex flex-col items-center">
        {avatar && (
            ...
            {footer && (
              <div
                className={cn(
                  ...
                  content && "mt-3 border-t border-br pt-3"
                )}
              >
