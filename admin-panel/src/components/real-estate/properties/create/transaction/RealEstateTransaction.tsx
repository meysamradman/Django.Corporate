...
interface RealEstateTransactionProps {
...
export function RealEstateTransaction(props: RealEstateTransactionProps) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <FormField
                label="وضعیت معامله"
                required
                error={isFormApproach ? formErrors.status?.message : formErrors?.status}
            >
                <Select
                ...
                >
                    <SelectTrigger className={cn((isFormApproach ? formErrors.status?.message : formErrors?.status) && "border-red-1")}>
