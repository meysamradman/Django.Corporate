...
export function RealEstateAgency({ agency, hasAgentOrCreator, compact = false }: RealEstateAgencyProps) {
    ...
    return (
        <div className={cn("flex-1 p-3", !compact && !hasAgentOrCreator && "lg:px-20 py-8")}>
            <Item
                className={cn(
                    "hover:bg-bg/5 transition-colors rounded-2xl p-3 gap-5 border-none",
                    compact && "bg-bg/35 ring-1 ring-blue-1/8"
                )}
            >
                    ...
                    >
                        <AvatarImage
                            src={
                                ...
                                (agency.logo_url
                                    ? mediaService.getMediaUrlFromObject({ file_url: agency.logo_url } as any)
                                    : undefined)
                            }
                ...
                <ItemContent className="min-w-0 flex-1">
                    <div className="flex items-center gap-2.5 mb-1.5">
                        <Badge
                            variant="blue"
                            className="text-[10px] font-black px-2 h-5 bg-blue-1/10 text-blue-1 border-none tracking-widest uppercase"
                        >
                            آژانس
                        </Badge>
                ...
                <ItemActions>
                    <Button
                        ...
                        className={cn(
                            "size-10 rounded-2xl text-blue-1/40 hover:text-blue-1 hover:bg-blue-1/10 border border-br/60 bg-wt transition-all shadow-3xs",
                            compact && "bg-blue-1/5 border-blue-1/20"
                        )}
                    >
