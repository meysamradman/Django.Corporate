...
import { Plus, Edit, Trash2, Link2, CheckCircle2, XCircle } from "lucide-react";
...
export function FooterLinks() {
    ...
    const { data: links = [], isLoading } = useQuery({
        ...
        queryFn: () => settingsApi.getFooterLinks(),
    });
    ...
    const handleOpenSide = (id?: number) => {
        openDrawer(DRAWER_IDS.SETTINGS_FOOTER_LINK_FORM, { editId: id });
    };
    ...
    return (
        <>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-bg/50 hover:bg-bg/50">
                                    ...
                                    <TableHead className="text-right">ستون</TableHead>
                            ...
                            <TableBody>
                                {links
                                    ...
                                    .map((link) => (
                                        <TableRow key={link.id} className="hover:bg-bg/50 transition-colors">
                                            <TableCell className="text-right font-medium">
                                                {link.title}
                                            </TableCell>
                                            <TableCell className="text-right">
                                                <span className="text-xs text-font-s">
                                                    {sectionTitleById.get(link.section) || "-"}
                                                </span>
                                            </TableCell>
                                            <TableCell className="text-right">
                                                <span className="text-xs text-font-s break-all">
                                                    {link.href}
                                                </span>
                                            </TableCell>
                                            ...
                                            <TableCell className="w-15">
                                                <div className="flex items-center justify-center">
                                                    <DataTableRowActions
                                                        row={{ original: link } as any}
                                                        actions={[
                                                            {
                                                                label: "ویرایش",
                                                                icon: <Edit className="h-4 w-4" />,
                                                                onClick: () => handleOpenSide(link.id),
                                                            },
                                                            {
                                                                label: "حذف",
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>حذف لینک فوتر</AlertDialogTitle>
                        <AlertDialogDescription>
                            آیا از حذف این لینک اطمینان دارید؟ این عمل غیرقابل بازگشت است.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
