...
import { X, AlertCircle, Image as ImageIcon, Globe, UploadCloud } from "lucide-react";
import type { BlogFormValues } from "@/components/blogs/validations/blogSchema";
import type { Media } from "@/types/shared/media";
import type { ChangeEvent } from "react";

interface BlogSEOSocialPreviewProps {
...
export function BlogSEOSocialPreview({
    isFormApproach,
    form,
    editMode,
    blogId,
                ...
                >
                        ...
                        <div className="relative w-full aspect-video rounded-lg overflow-hidden group border">
                            ...
                            <div className="absolute inset-0 bg-static-b/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                <Button
                                    ...
                                    className="mx-1 hover:bg-white hover:text-black"
                                    disabled={!editMode}
                                >
                                    تغییر تصویر
                                </Button>
                    ...
                    {(formState?.errors as any)?.og_image?.message && (
                        <div className="flex items-start gap-2 text-red-2 mt-2">
                            ...
                            <span>{String(formState?.errors ? (formState.errors as any).og_image?.message : "")}</span>
                        </div>
                    )}
                </CardWithIcon>
            </div>
