...
export function BlogSEO({ blog }: BlogSEOProps) {
    const ogImageUrl = blog.og_image?.file_url
        ? mediaService.getMediaUrlFromObject({ file_url: blog.og_image.file_url } as any)
        : "";

    const hasSEO = blog.meta_title || blog.meta_description || blog.og_title || blog.og_description || blog.og_image;

    const metaTitle = blog.meta_title || blog.title || "بدون عنوان سئو";
    const metaDescription = blog.meta_description || blog.short_description || "بدون توضیحات سئو";
        ...
        >
            {!hasSEO ? (
                <EmptyState
                    ...
                    description="توضیحات متا و پیش‌نمایش شبکه‌های اجتماعی برای این وبلاگ ثبت نشده است"
                    icon={Globe}
                    size="sm"
                    fullBleed={true}
                />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-stretch">
                    <div className="flex flex-col h-full gap-5">
                        <Card className="flex-1 gap-0 bg-bg/40 border-br/40 shadow-xs group transition-smooth hover:border-teal-1/30 overflow-hidden relative">
