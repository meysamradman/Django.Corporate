...
class PortfolioPrintListExportService:
    """
    Service to generate a print-friendly HTML view for Portfolio listings.
    """
    ...
    @staticmethod
    def export_portfolios_print(queryset):
        html_content = [
            "<!DOCTYPE html>",
            ...
            PortfolioPrintListExportService._get_css(),
            "</head>",
            "<body>",
            ...
            "<div class='header'>",
            f"<h1>{PDF_LABELS.get('portfolios_list', 'لیست نمونه کارها')}</h1>",
            ...
            f"<th>{PDF_LABELS.get('options', 'گزینه‌ها')}</th>",
        ...
        for portfolio in queryset:
            ...
            status_text = status_map.get(portfolio.status, portfolio.status)
            
            categories = ", ".join([c.name for c in portfolio.categories.all()[:3]])
            tags = ", ".join([t.name for t in portfolio.tags.all()[:3]])
            options = ", ".join([o.name for o in portfolio.options.all()[:3]])
            created_at = format_jalali_date(portfolio.created_at)
            is_featured = "بله" if portfolio.is_featured else "خیر"
