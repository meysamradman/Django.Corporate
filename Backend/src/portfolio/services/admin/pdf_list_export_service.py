...
class PortfolioPDFListExportService:
    """Service for exporting portfolio list to PDF format (table format, similar to Excel)"""
    ...
    def export_portfolios_pdf(queryset):
        ...
        try:
            ...
            pdf_styles = PortfolioPDFListExportService._create_persian_styles(persian_font_name)
            ...
            for row_idx in range(1, len(table_data)):  # Skip header
                ...
                if len(row) > 0:
                    ...
                    status_para = Paragraph(
                        f'<para align="right"><font name="{persian_font_name}" size="10" color="{PortfolioPDFListExportService.TEXT_PRIMARY}">'
                        f'{status_text}'
                        f'</font></para>',
                        normal_style
                    )
            ...
            logger.error(f"PDF list export error: {e}\n{traceback.format_exc()}")
