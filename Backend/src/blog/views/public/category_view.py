...
class BlogCategoryPublicViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if tree_mode:
            data = BlogCategoryPublicService.get_tree_data_serialized()
        ...
        data = BlogCategoryPublicService.get_category_list_data(filters=filters, search=search, ordering=ordering)
    ...
    def retrieve(self, request, *args, **kwargs):
        category_data = BlogCategoryPublicService.get_category_detail_by_slug_data(kwargs.get('slug'))
        if category_data:
            return APIResponse.success(
    ...
    def get_by_public_id(self, request, public_id=None):
        category_data = BlogCategoryPublicService.get_category_detail_by_public_id_data(public_id)
        if category_data:
            return APIResponse.success(
                message=CATEGORY_SUCCESS['category_retrieved'],
                data=category_data,
    ...
    @action(detail=False, methods=['get'], url_path='id/(?P<category_id>[^/.]+)')
    def get_by_id(self, request, category_id=None):
        try:
        ...
        category_data = BlogCategoryPublicService.get_category_detail_by_id_data(parsed_id)
        if category_data:
            return APIResponse.success(
                message=CATEGORY_SUCCESS['category_retrieved'],
                data=category_data,
    ...
    def roots(self, request):
        data = BlogCategoryPublicService.get_root_categories_serialized()
        return APIResponse.success(
