...
class BlogAdminService:
    ...
    def get_list_cache_key(user_id, query_params):
        payload = {
            'user_id': user_id,
            'query_params': BlogAdminService._normalize_query_params(query_params),
        }
        return BlogCacheKeys.list_admin(payload)

    @staticmethod
    def get_cached_list_payload(user_id, query_params):
        cache_key = BlogAdminService.get_list_cache_key(user_id, query_params)
        return BlogCacheManager.get(cache_key)

    @staticmethod
    def set_cached_list_payload(user_id, query_params, payload):
        cache_key = BlogAdminService.get_list_cache_key(user_id, query_params)
        BlogCacheManager.set(cache_key, payload, timeout=cache_ttl.ADMIN_LIST_TTL)
...
class BlogExportRateLimitService:
    ...
    def check_and_increment(user_id, limit, window_seconds):
        cache_key = f"admin:blog:export:limit:{user_id}"
