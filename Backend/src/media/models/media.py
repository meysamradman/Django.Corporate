    ...
    def clean(self):
        ...
        if ext not in ALLOWED_EXTENSIONS['image']:
            raise ValidationError(
                MEDIA_ERRORS['invalid_image_extension'].format(
                    extensions=', '.join(ALLOWED_EXTENSIONS['image'])
                )
            )
...
class AbstractVideoMedia(AbstractMedia):
    ...
    def clean(self):
        ...
        if ext not in ALLOWED_EXTENSIONS['video']:
            raise ValidationError(
                MEDIA_ERRORS['invalid_video_extension'].format(
                    extensions=', '.join(ALLOWED_EXTENSIONS['video'])
                )
            )
...
class AbstractAudioMedia(AbstractMedia):
    ...
    def clean(self):
        ...
        if ext not in ALLOWED_EXTENSIONS['audio']:
            raise ValidationError(
                MEDIA_ERRORS['invalid_audio_extension'].format(
                    extensions=', '.join(ALLOWED_EXTENSIONS['audio'])
                )
            )
...
class AbstractDocumentMedia(AbstractMedia):
    ...
    def clean(self):
        ...
        if ext not in ALLOWED_EXTENSIONS['pdf']:
            raise ValidationError(
                MEDIA_ERRORS['invalid_document_extension'].format(
                    extensions=', '.join(ALLOWED_EXTENSIONS['pdf'])
                )
            )
