...
def validate_form_submission(form_data, platform):
    if not isinstance(form_data, dict):
        raise ValidationError("form_data must be a dictionary")
    ...
    for field in fields:
        ...
        if field.required:
            if not value or (isinstance(value, str) and not value.strip()):
                errors[field_key] = f"Field '{field.label}' is required"
        if value:
            if field.field_type == 'email':
                if '@' not in str(value):
                    errors[field_key] = f"Invalid email"
            elif field.field_type == 'phone':
                if not str(value).replace(' ', '').replace('-', '').isdigit():
                    errors[field_key] = f"Invalid phone number"
            elif field.field_type == 'number':
                try:
                    float(value)
                except (ValueError, TypeError):
                    errors[field_key] = f"Invalid number"
            elif field.field_type in ['select', 'radio']:
                valid_options = [opt['value'] for opt in (field.options or [])]
                if value not in valid_options:
                    errors[field_key] = f"Invalid selected option"
            if field.validation_rules:
                rules = field.validation_rules
                value_str = str(value)
                if 'min_length' in rules and len(value_str) < rules['min_length']:
                    errors[field_key] = f"Minimum {rules['min_length']} characters required"
                if 'max_length' in rules and len(value_str) > rules['max_length']:
                    errors[field_key] = f"Maximum {rules['max_length']} characters allowed"
                if 'pattern' in rules:
                    if not re.match(rules['pattern'], value_str):
                        errors[field_key] = "Invalid format"
    return {
        'valid': len(errors) == 0,
        'errors': errors
    }
...
def create_contact_form_submission(validated_data):
    try:
        ...
        for field in fields:
            ...
            elif field.field_key in ['name', 'نام'] and name == 'No Name':
                name = str(value) if value else 'No Name'
        if name == 'No Name':
            name = form_data.get('name', form_data.get('نام', 'No Name'))
        if not email:
        ...
        subject = ' | '.join(subject_parts) if subject_parts else 'New Contact Form'
        if len(subject) > 300:
            subject = subject[:297] + '...'
        ...
        raise ValidationError(f"Error creating submission: {str(e)}")
