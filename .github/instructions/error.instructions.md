...
## ✅ آپدیت اجرایی برای پروژه Corporate (واقعی و الزام‌آور)

برای اینکه تیم همیشه خروجی یکسان بدهد، این قواعد روی کد فعلی پروژه **الزامی** هستند:

1. **هیچ پیام hardcode در real_estate مجاز نیست**
    - تمام متن‌های خطا باید از `src/real_estate/messages/messages.py` خوانده شود.

2. **Serializer اولویت اول Validation است**
    - `validate_<field>` و `validate` محل اصلی پیام کاربر هستند.

3. **Model-level invariant مجاز است (فقط برای قیود ذاتی مدل)**
    - مثل `clean()` برای `only_one_main_image`.
    - حتی در مدل هم متن خطا باید از فایل messages بیاید، نه متن انگلیسی/هاردکد.

4. **Service مجاز به APIResponse/message hardcode نیست**
    - Service باید exception محور باشد.
    - اگر مجبور به raise شد، پیام باید از messages constants بیاید.

5. **View باید خطای race دیتابیس را map کند**
    - برای `IntegrityError` (یونیک‌ها مثل `slug/title/license_number`) حتماً mapping به پیام فارسی استاندارد انجام شود.
    - خروجی نهایی فقط از `APIResponse` و `normalize_validation_error` عبور کند.

6. **UniqueValidator پیش‌فرض DRF در serializerهای حساس باید کنترل شود**
    - اگر باعث خروجی انگلیسی شود، validator پیش‌فرض حذف/override شود تا پیام ثابت فارسی برگردد.

---

## 🧪 چک‌لیست سریع PR (الزامی قبل از مرج)

- [ ] هیچ `ValidationError("...")` هاردکد در `src/real_estate` وجود ندارد.
- [ ] هیچ `APIResponse.error(message="...")` هاردکد در viewهای `real_estate` وجود ندارد.
- [ ] پیام‌های یکتایی (`slug/title/...`) در create/update فارسی و ثابت هستند.
- [ ] مدل‌های دارای `clean()` از constants پیام استفاده می‌کنند.
- [ ] سناریوی race condition (IntegrityError) در view map شده است.
